/********************************************************
 * GreedyRank_core.cpp
 * C++ implements on GreedyRank [Zheng et al, SRDS'2010]
...
********************************************************/
void GreedyRank_core(double *removedData, int numUser, int numService, int topK, int *predData)
{	
    ...
    for (i = 0; i < numUser; i++) {
		...
		greedyRank(removedMatrix[i], preferenceMatrix, predRankMatrix[i], numService, false);
    }
