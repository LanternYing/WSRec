...
def predict(matrix, density, paraStruct):
    ...
    for k in range(rounds):
		...
		predRankMatrix = GreedyRank.GreedyRank_wrapper(trainMatrix, paraStruct)  		
		timeResults[k] = time.clock() - iterStartTime

		# calculate the prediction error
		evalResults.append(errMetric(matrix, predRankMatrix, paraStruct['metrics']))

		logger.info('%d-round done. Running time: %.2f sec'%(k + 1, timeResults[k]))
		logger.info('----------------------------------------------')

    outFile = '%s%.2f.txt'%(paraStruct['outPath'], density)
    saveResult(outFile, np.array(evalResults), timeResults, paraStruct)
