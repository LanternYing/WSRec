...
#########################################################
# Function to perform CloudPred, as a wrapper of CloudPred_core
# return the predicted matrix
#
def CloudPred(removedMatrix, paraStruct):  
    cdef int numService = removedMatrix.shape[1] 
    cdef int numUser = removedMatrix.shape[0] 
    cdef int dim = paraStruct['dimension']
    cdef int maxIter = paraStruct['maxIter']
    ...
    # Perform NMF
    (U, S) = NMF(removedMatrix, paraStruct)
    logger.info('NMF phase done.')

    # Perform UIPCC          
    uMean = UMEAN(removedMatrix)    
    iMean = IMEAN(removedMatrix)     
    predMatrixUPCC = UPCC(removedMatrix, U, uMean, paraStruct)
    logger.info('UPCC phase done.')  
    predMatrixIPCC = IPCC(removedMatrix, S, iMean, paraStruct)
    logger.info('IPCC phase done.')
    predMatrixUIPCC = UIPCC(predMatrixUPCC, predMatrixIPCC, paraStruct)
