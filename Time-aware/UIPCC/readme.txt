...
UIPCC/
  ...
  - scripts/
      - resultHandler.py        - a script to caculate the average values over 
  - result/                     - directory for storing evaluation results
                                  available metrics: (MAE, NMAE, RMSE, MRE, NPRE)
      - 01_relResult_0.05.txt   - E.g., the reponse-time prediction result under 
                                  matrix density = 5%
      - [...]                   - many other results
      - average/
          ...
          - UIPCC/              - the average results for UIPCC approach
              - avg_relResult_0.05.txt   - the average result under matrix density 
                                           = 5%
		      - [...]                    - other results		  
...
approaches on the dataset. E.g.,"result/01_relResult_0.05.txt" records the 
evaluation results for the first time slice under matrix density = 5%. In 
particular, each experiment is run for 20 times and the average result 
(including std value) is reported. These results can be directly used for 
your research work.
